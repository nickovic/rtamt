...
class TestIssue84(unittest.TestCase):
    def __init__(self, *args, **kwargs):
        super(TestIssue84, self).__init__(*args, **kwargs)
        #self.spec = '(not ((always[0.0, 4.0]((x <= 250.0) and (x >= 240.0))) and (eventually[3.5,4.0]((x <= 240.1) and (x >= 240.0)))))'

        #self.spec = 'always[0.0, 4.0] ((x <= 250.0) and (x >= 240.0))'

        self.spec = 'eventually[3.5s,4.0s]((x <= 240.1) and (x >= 240.0))'
    ...
    def test_issue_84_discrete_offline_with_pastification(self):
        """
        Discrete Offline Monitor (w/ Pastification)
        """

        print('DISCRETE OFFLINE WITH PASTIFICATION')
        phi = rtamt.STLDiscreteTimeSpecification()
    ...
    def test_issue_84_discrete_offline_without_pastification(self):
        """
    ...
    def test_issue_84_dense_offline_with_pastification(self):
        """
        Dense Offline Monitor (w/ Pastification)
        """

        print('DENSE OFFLINE WITH PASTIFICATION')
        phi = rtamt.STLDenseTimeSpecification()
    ...
    def test_issue_84_dense_offline_without_pastification(self):
        """
